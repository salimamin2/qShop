...
jQuery(document).ready(function() {
	...
	jQuery('#pages a').each(function(index) {
		pages[index] = jQuery(this).attr('href');
		loaded[jQuery(this).attr('href')] = 0; //initialize all the pages to be loaded to 0. It means that they are not yet been loaded.
	});
	...
	function loadMoreContent(position) {
		...
		if(position < pages.length) {
			jQuery('#loader').fadeIn('slow', function() {
				jQuery.get(pages[position], function(data) {
