...
class ControllerAccountAddress extends Controller {
    ...
    public function insert() {
		if (!$this->customer->isLogged()) {
		    $this->session->data['redirect'] = makeUrl('account/address', array(), true, true);
		    $this->redirect(makeUrl('account/login', array(), true, true));
		}
		$this->language->load('account/address');
		$this->document->title = $this->language->get('heading_title');
	...
	$this->document->breadcrumbs[] = array(
	    'href' => makeUrl('account/address', array(), true, true),
...
//                $format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
		...
		'update' => makeUrl('account/address/update', array('address_id=' . $result['address_id']), true, true),
		'delete' => makeUrl('account/address/delete', array('address_id=' . $result['address_id']), true, true)
	    );
	}

	$this->data['insert'] = makeUrl('account/address/insert', array(), true, true);
	...
	if (!isset($this->request->get['address_id'])) {
	    $this->document->breadcrumbs[] = array(
		'href' => makeUrl('account/address/insert', array(), true, true),
		'text' => $this->language->get('text_edit_address'),
		'separator' => FALSE
	    );
	    $this->data['heading_title'] = $this->language->get('heading_title') . ' &raquo; ' . $this->language->get('text_add_address');
	} else {
	    $this->document->breadcrumbs[] = array(
		'href' => makeUrl('account/address/update', array('address_id=' . $this->request->get['address_id']), true, true),
		'text' => $this->language->get('text_edit_address'),
		'separator' => FALSE
	    );
	    $this->data['heading_title'] = $this->language->get('heading_title') . ' &raquo; ' . $this->language->get('text_edit_address');
	}
	...
	if (!isset($this->request->get['address_id'])) {
	    $this->data['action'] = makeUrl('account/address/insert', array(), true, true);
	} else {
	    $this->data['action'] = makeUrl('account/address/update', array('address_id=' . $this->request->get['address_id']), true, true);
	}

	if (isset($this->request->get['address_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
	    $address_info = $this->model_account_address->getAddress($this->request->get['address_id']);
	    $this->request->post['zone_id'] = $address_info['zone_id'];
	}
	...
	if (isset($this->request->post['zone_id'])) {
	    ...
	    $zone_id = $this->data['zone_id'] = $address_info['zone_id'];
	} else {
	...
	if (isset($this->request->post['address_type'])) {
	    $this->data['address_type'] = $this->request->post['address_type'];
	} elseif (isset($address_info)) {
	    $this->data['address_type'] = $address_info['address_type'];
	} else {
	...
	$this->document->addScriptInline('var dataForm = new VarienForm("address",true);', Document::POS_END);
    ...
    if ((strlen(utf8_decode(trim($this->request->post['company']))) < 3) || (strlen(utf8_decode(trim($this->request->post['company']))) > 32)) {
        $this->error['lastname'] = $this->language->get('error_telephone');
    }

	if ((strlen(utf8_decode(trim($this->request->post['address_1']))) < 3) || (strlen(utf8_decode(trim($this->request->post['address_1']))) > 128)) {
	    $this->error['address_1'] = $this->language->get('error_address_1');
	}

	if ((strlen(utf8_decode(trim($this->request->post['city']))) < 3) || (strlen(utf8_decode(trim($this->request->post['city']))) > 128)) {
	    $this->error['city'] = $this->language->get('error_city');
	}

	if (!isset($this->request->post['default']) && $this->customer->getAddressId() == $this->request->get['address_id']) {
	    $this->error['default'] = $this->language->get('error_default_1');
	}
	...
	if ($this->request->post['country_id'] == 'FALSE') {
	    ...
	    $this->load->model('localisation/country');
	    $aCountry = $this->model_localisation_country->getCountry($this->request->post['country_id']);
	    if ($aCountry['iso_code_2'] == 'US' && (strlen(trim($this->request->post['postcode'])) < 3) || (strlen(trim($this->request->post['postcode'])) > 10))
