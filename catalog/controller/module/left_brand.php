...
class ControllerModuleLeftBrand extends Controller {

    protected function index() {
        $this->language->load('module/left_brand');
...
//        $this->data['parentTitle'] = Make::a('catalog/category')->create()->getCategoryTitle($cat_id);
        ...
        $categories_id = implode(',',unserialize($this->config->get('config_category_brand')));
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/left_brand.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/module/left_brand.tpl';
        } else {
            $this->template = 'default/template/module/left_brand.tpl';
        }
