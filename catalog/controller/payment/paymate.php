...
class ControllerPaymentPaymate extends Controller {
	protected function index() {
		...
		$this->data['return'] = HTTPS_SERVER . 'payment/paymate/callback&oid=' . base64_encode($encryption->encrypt($order_info['order_id'])) . '&conf=' . base64_encode($encryption->encrypt($order_info['payment_firstname'] . $order_info['payment_lastname']));
	...
	public function callback() {
		...
		if ($error != '') {
			...
			$this->data['text_message'] = sprintf($this->language->get('text_failed_message'), $error, HTTP_SERVER . 'information/contact');
			...
			$this->redirect(HTTPS_SERVER . 'checkout/success');
		}
	}
}
?>
