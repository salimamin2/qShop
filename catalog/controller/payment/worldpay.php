...
class ControllerPaymentWorldPay extends Controller {
	...
	public function callback() {
		if (isset($this->request->post['callbackPW']) && ($this->request->post['callbackPW'] == $this->config->get('worldpay_password'))) {
			...
			$this->data['text_success_wait'] = sprintf($this->language->get('text_success_wait'), HTTPS_SERVER . 'checkout/success');
			$this->data['text_failure'] = $this->language->get('text_failure');
			
			if ($this->request->get['act'] != 'checkout/guest_step_3') {
				$this->data['text_failure_wait'] = sprintf($this->language->get('text_failure_wait'), HTTPS_SERVER . 'checkout/payment');
			} else {
				$this->data['text_failure_wait'] = sprintf($this->language->get('text_failure_wait'), HTTPS_SERVER . 'checkout/guest_step_2');
			}
			...
			if (isset($this->request->post['transStatus']) && $this->request->post['transStatus'] == 'Y') { 
    			...
    			$this->data['continue'] = HTTPS_SERVER . 'checkout/cart';
