...
class ControllerPaymentSagepay extends Controller {
	protected function index() {
		...
		$data['SuccessURL'] = HTTPS_SERVER . 'payment/sagepay/success&order_id=' . $this->session->data['order_id'];
		
		if ($this->request->get['act'] != 'checkout/guest_step_3') {
			$this->data['FailureURL'] = HTTPS_SERVER . 'checkout/payment';
		} else {
			$this->data['FailureURL'] = HTTPS_SERVER . 'checkout/guest_step_2';
		}
	...
	public function success() {
		if (isset($this->request->get['crypt'])) {
			...
			if ($data) {
				...
				$this->redirect(HTTP_SERVER . 'checkout/success');
			}
		}
	}	 
