...
//			mb_internal_encoding($this->CI->config->get('charset'));
//		}
...
//		$this->CI->lang->load('form_validation');
        $this->CI->load->language('common/form_validation');
        ...
        if (is_null($postdata) AND $callback == FALSE) {
            if (in_array('isset', $rules, TRUE) OR in_array('required', $rules)) {
                ...
                if (!isset($this->_error_messages[$type])) {
                    if (FALSE === ($line = $this->CI->language->get($type))) {
        ...
        foreach ($rules As $rule) {
            ...
            if ($result === FALSE) {
                if (!isset($this->_error_messages[$rule])) {
                    if (FALSE === ($line = $this->CI->language->get($rule))) {
                ...
                // Build the error message
                if (isset($match[2]))
                    $message = sprintf($line, $this->_translate_fieldname($row['label']), $param, $match[2]);
                else
    ...
    function _translate_fieldname($fieldname) {
        ...
        if (substr($fieldname, 0, 5) == 'lang:') {
            ...
            if (FALSE === ($fieldname = $this->CI->language->get($line))) {
                return $line;
            }
        }
