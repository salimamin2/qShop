...
class PHPMailer {
    ...
    public function Send() {
        try {
            ...
            switch ($this->Mailer) {
                ...
                case 'sendgrid':
                    return $this->GridSend($header, $body);
                default:
    ...
    public function GridSend($header, $body) {
        try {
            require_once('sendgrid/autoload.php');
            ...
            if (!$this->Username || !$this->Password) {
                throw new phpmailerException('Error: No username or password given for SendGrid', self::STOP_CRITICAL);
            }
            ...
            if ($aAttachments) {
                ...
                foreach ($aAttachments as $aAttach) {
                    $filename = $aAttach[0];
                    if ($aAttach[5]) {
                        $filename = DIR_DOC . $aAttach[1];
                        file_put_contents($filename, $aAttach[0]);
                    }
                    $email->setAttachment($filename, $aAttach[1]);
                }
            }
            //d($email,true);
            $response = $sendgrid->send($email);
            if (property_exists($response, 'message') && $response->message == 'error') {
                throw new phpmailerException($response->errors[0], self::STOP_CRITICAL);
            }
            return $response;
        } catch (Exception $e) {
    ...
    public function CreateHeader() {
        $result = '';
        $result .= $this->HeaderLine("Organization", QS::app()->config->get('config_name'));
        ...
        $result .= $this->HeaderLine("X-AntiAbuse", "This is a solicited email for - " . QS::app()->config->get('config_name') . " mailing list.");
