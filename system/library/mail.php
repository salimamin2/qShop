...
final class Mail {
	...
	/*public function __construct(){
		$this->registery = Registery::getInstance();
		$this->config = $this->registery->get('config');
	}*/
    ...
    public function setSmsText($setsmstext) {
        $this->setsmstext = $setsmstext;
    }
	...
	public function send(){
		try{
			$mail = new PHPMailer();
	        if (QS::app()->config->get('config_smtp_ssl') == '1')
	            $mail->SMTPSecure = 'ssl';//'tls'//QS::app()->config->get('config_smtp_secure'); //New line Added
	       ...
	       // if (QS::app()->config->get('config_smtp_auth') == '1')
	        ...
	        $mail->send();
	        QS::d(array('Mail-'.date('Y-m-d H:i:s').': ' => $mail),0,0,1);
    	} catch(Exception $ex){
    		QS::d('Mail Error: '.$ex->getMessage(),0,0,1);
    	}

	}

    public function sendSms($sms_message,$smsnumber,$id,$pass,$mask){

//d($this->config->get);
       /* $sms_username =$username;
        $sms_password = $password;
        $sms_mask = $mask;
        $client = new SoapClient("http://180.178.135.203:9735/soap/index.php?wsdl");
        $params = array(
        ...
        $response = $client->__call('SMS_Services.SendSMS',$params);
        ...
        $result = curl_exec($ch);
        //print_r($result);
		//This is the result from Outreach
        d(array('request'=>$data,'response'=>$result),0,0,1);
        curl_close($ch);
        return true;//$result;
    }
	...
	/*public function send(){
		...
		$header .= "From: $this->sender <$this->sender> \r\n";
		$header .= "CC: \r\n";
		$header .= "BCC: \r\n ";	
		if($this->text !="")	
			$message = $this->text;
