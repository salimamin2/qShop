...
class Validator {
	...
	protected $asMessage = array( 'required' => '{name} cannot be blank', 'date' => '{name} must be valid date.', 'digits' => '{name} must be valid number.', 'email' => '{name} is not a valid email.', 'equalTo' => '{name} and {name2} value not are not same.', 'max' => '{name} must be less then {max}', 'min' => '{name} must be greater then {min}', 'maxlength' => '{name} must be less then {max} charecters', 'minlength' => '{name} must be greater then {min} charecters', 'url' => '{name} invalid url', 'range' => '{name} must be between {range1} and {range2}', 'accept' => '{name} has invalid files, only {types} accepted', );
	...
	*/
	...
	public function isSafeAttributes($scenario, $attribute) {
		if ( empty ( $this->on ) || isset ( $this->on['scenario'] ))
			...
			if ( in_array($this->on['scenario'], $attribute))
				return true;
			return false;
	}
	...
	*/
	...
	public function __construct($object, array $asOption = null) {
		...
		if ( false === is_null($asOption)) {
			...
			if ( isset ( $asOption['messages'] ) && is_array($asOption['messages'])) {
	...
	*/
	...
	private function getMessage($sField, $sRule, $asRuleParam) {
		if ( isset ( $this->asMessage[$sField] ) && is_array($this->asMessage[$sField]) && isset ( $this->asMessage[$sField][$sRule] )) {
			...
			if ( isset ( $this->asMessage[$sField] ) && isset ( $this->asMessage[$sField][$sRule] )) {
	...
	*/
	...
	public function optional($sField) {
		return false === ( isset ( $this->asClassRule[$sField] ) && isset ( $this->asClassRule[$sField]['required'] ));
	}
	...
	*/
	...
	public function showError($sField) {
		if ( isset ( $this->asErrorMessage[$sField] )) {
			return '<' . $this->sErrorElement . ( $this->sErrorElement === 'label' ? ' ' : ' html' ) . 'for="' . $sField . '" generated="true" class="' . $this->sErrorClass . '">' . $this->asErrorMessage[$sField] . '</' . $this->sErrorElement . '>';
		}
	}
	...
	*/
	...
	public function hasErrors($feild = null) {
		if ( $field === null )
	...
	*/
	...
	public function showValid($sField) {
		return $this->valid($sField) && isset ( $this->asValue[$sField] ) ? $this->asValue[$sField] : '';
	}
	...
	*/
	...
	public function valid($sField = null) {
		return is_null($this->asErrorMessage) ||
		# not validated
		count($this->asErrorMessage) === 0 ||
		# no errors
		(
		# there are errors: can now only return true when
		false === is_null($sField) &&
		# - specific field is requested
		false === isset ( $this->asErrorMessage[$sField] )
		# -	and no error for this field
		);
	}
	...
	*/
	...
	public function validate(array $asValue) {
		...
		foreach ( $this->asClassRule as $sField => $asRules ) {
			if ( false === isset ( $asValue[$sField] ) &&
				# this field was not validatied, as it was not present in the list
				isset ( $asRules['required'] )) {
				# But is was REQUIRED ! !
	...
	public function element($sField, $mValue) {
		...
		if ( isset ( $this->asClassRule[$sField] )) {
			...
			foreach ( $this->asClassRule[$sField] as $sRule => $asRuleParam ) {
				if ( false === (
					# iff empty AND optional: don't check
					empty ( $mValue ) &&
					# easier checked than next line
					$this->optional($sField)) && false === $this->oMethodCollection->{ $sRule } ( $mValue, $asRuleParam )) {
  ...
  (
	...
	*/
	...
	public function __call($sName, array $asParam) {
		if ( isset ( $this[$sName] )) {
			return $this[$sName] ( array_shift($asParam), array_shift($asParam));
			# sValue, asRuleParam
		}
		# no validation set...
		else {
	...
	*/
	...
	public function email($email, $mOption) {
		$isValid = true;
	...
	*/
	...
	public function minlength($sValue, $iLength) {
		...
		# pre-decrement: zero-indexed char-array
	}
	...
	public function required($sValue, $mParam = null) {
		if ( false === is_null($mParam)) {
			...
			elseif ( is_string($mParam)) {
				...
				# NOT required iff:
				if ( false === isset ( $_POST[$oJQuery->sFieldname] ) ||
					# 	the field is not there (i.e. not checked)
					false === empty ( $_POST[$oJQuery->sFieldname] )) {
					# 	the field is empty (i.e. select/input)
					return true;
					# not required means valid!
				}
			}
		}
