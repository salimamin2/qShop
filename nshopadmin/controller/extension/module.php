            ...
            foreach ($files as $file) {
                ...
                if (!in_array($extension, $extensions)) {
                    $action[] = array(
                        ...
                        'class' => 'btn-success',   
                        // 'href' => HTTPS_SERVER . 'extension/module/install&token=' . $this->session->data['token'] . '&extension=' . $extension
                        'href' => makeUrl('extension/module/install',array('extension=' . $extension))
                    );
                } else {
                    $action[] = array(
                        ...
                        'icon' => 'icon-pencil',
                        'class' => 'btn-info',   
                        // 'href' => HTTPS_SERVER . 'module/' . $extension . '&token=' . $this->session->data['token']
                        'href' => makeUrl('module/' . $extension)
                    );
                    ...
                    $action[] = array(
                        ...
                        'class' => 'btn-danger',                        
                        // 'href' => HTTPS_SERVER . 'extension/module/uninstall&token=' . $this->session->data['token'] . '&extension=' . $extension
                        'href' => makeUrl('extension/module/uninstall',array('extension=' . $extension))
                    );
                }
...
//            $this->load->model('user/user_group');
            $oUserGroup = Make::a('user/user_group')->create();

            $oUserGroup->addPermission($this->user->getId(), 'access', 'module/' . $this->request->get['extension']);
            $oUserGroup->addPermission($this->user->getId(), 'modify', 'module/' . $this->request->get['extension']);
