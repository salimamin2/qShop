...
class ControllerExtensionPayment extends Controller {
			...
			foreach ($files as $file) {
				...
				if (!in_array($extension, $extensions)) {
					$action[] = array(
						...
						'href' => HTTPS_SERVER . 'extension/payment/install?extension=' . $extension
					);
				} else {
					$action[] = array(
						...
						'href' => HTTPS_SERVER . 'payment/' . $extension
					);
					...
					$action[] = array(
						...
						'href' => HTTPS_SERVER . 'extension/payment/uninstall&token=' . $this->session->data['token'] . '&extension=' . $extension
					);
				}
	...
	public function install() {
		if (!$this->user->hasPermission('modify', 'extension/payment')) {
			...
			$this->redirect(makeUrl('extension/payment'));
		} else {
			$this->load->model('setting/extension');
            ...
            $oModel = Make::a('user/user_group')->create();

            $oModel->addPermission($this->user->getId(), 'access', 'payment/' . $this->request->get['extension']);
            $oModel->addPermission($this->user->getId(), 'modify', 'payment/' . $this->request->get['extension']);
