...
class ControllerModuleBestSeller extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
            Make::a('setting/setting')->create()->editSetting('bestseller', $this->request->post);		
			$this->cache->delete('product');
			$this->data['success'] = $this->language->get('text_success');
			// $this->session->data['success'] = $this->language->get('text_success');
			// $this->redirect(makeUrl('module/');
		}
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'module/bestseller&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = HTTPS_SERVER . 'module/bestseller&token=' . $this->session->data['token'];
