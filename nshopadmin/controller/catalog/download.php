	...
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect(HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . $url);
		}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = HTTPS_SERVER . 'catalog/download/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'catalog/download/delete&token=' . $this->session->data['token'] . $url;	
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => HTTPS_SERVER . 'catalog/download/update&token=' . $this->session->data['token'] . '&download_id=' . $result['download_id'] . $url
			);
		...
		$this->data['sort_name'] = HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . '&sort=dd.name' . $url;
		$this->data['sort_remaining'] = HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . '&sort=d.remaining' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . $url . '&page={page}';
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['download_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'catalog/download/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'catalog/download/update&token=' . $this->session->data['token'] . '&download_id=' . $this->request->get['download_id'] . $url;
		}
		
		$this->data['cancel'] = HTTPS_SERVER . 'catalog/download&token=' . $this->session->data['token'] . $url;
