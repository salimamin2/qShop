...
class ControllerFeedGoogleSitemap extends Controller {
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			Make::a('setting/setting')->create()->editSetting('google_sitemap', $this->request->post);				
		...
		$this->data['tab_general'] = $this->language->get('tab_general');
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'feed/google_sitemap&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = HTTPS_SERVER . 'feed/google_sitemap&token=' . $this->session->data['token'];
		...
		$this->data['data_feed'] = HTTP_CATALOG . 'feed/google_sitemap';
