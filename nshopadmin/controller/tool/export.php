...
class ControllerToolExport extends Controller { 
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if ($this->model_tool_export->upload($file)) {
					...
					$this->redirect(HTTPS_SERVER . 'tool/export');
				}
				else {
		...
		$this->document->breadcrumbs[] = array(
			'href'      => HTTPS_SERVER . 'tool/export',
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);
		
		$this->data['action'] = HTTPS_SERVER . 'tool/export';

		$this->data['export'] = HTTPS_SERVER . 'tool/export/download';
