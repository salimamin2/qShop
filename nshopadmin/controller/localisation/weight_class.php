...
class ControllerLocalisationWeightClass extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect(HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . $url);
		}
	...
	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['insert'] = HTTPS_SERVER . 'localisation/weight_class/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'localisation/weight_class/delete&token=' . $this->session->data['token'] . $url;
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => HTTPS_SERVER . 'localisation/weight_class/update&token=' . $this->session->data['token'] . '&weight_class_id=' . $result['weight_class_id'] . $url
			);
		...
		$this->data['sort_title'] = HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . '&sort=title' . $url;
		$this->data['sort_unit'] = HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . '&sort=unit' . $url;
		$this->data['sort_value'] = HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . '&sort=value' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . $url . '&page={page}';
	...
	private function getForm() {
		...
		if (!isset($this->request->get['weight_class_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'localisation/weight_class/insert&token=' . $this->session->data['token'] . $url;
		} else { 
			$this->data['action'] = HTTPS_SERVER . 'localisation/weight_class/update&token=' . $this->session->data['token'] . '&weight_class_id=' . $this->request->get['weight_class_id'] . $url;
		}

		$this->data['cancel'] = HTTPS_SERVER . 'localisation/weight_class&token=' . $this->session->data['token'] . $url;
