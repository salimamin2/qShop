	...
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['order'])) {
      		...
      		$this->redirect(HTTPS_SERVER . 'localisation/order_status&token=' . $this->session->data['token'] . $url);
		}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'localisation/order_status&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = HTTPS_SERVER . 'localisation/order_status/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'localisation/order_status/delete&token=' . $this->session->data['token'] . $url;	
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'icon' => 'icon-pencil',
                'class' => 'btn-info',
				'href' => HTTPS_SERVER . 'localisation/order_status/update&token=' . $this->session->data['token'] . '&order_status_id=' . $result['order_status_id'] . $url
			);
		...
		$this->data['sort_name'] = HTTPS_SERVER . 'localisation/order_status&token=' . $this->session->data['token'] . '&sort=name' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'localisation/order_status&token=' . $this->session->data['token'] . $url . '&page={page}';
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['order_status_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'localisation/order_status/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'localisation/order_status/update&token=' . $this->session->data['token'] . '&order_status_id=' . $this->request->get['order_status_id'] . $url;
		}
			
		$this->data['cancel'] = HTTPS_SERVER . 'localisation/order_status&token=' . $this->session->data['token'] . $url;
