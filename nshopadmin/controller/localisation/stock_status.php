	...
	private $error = array(); 
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['order'])) {
      		...
      		$this->redirect(HTTPS_SERVER . 'localisation/stock_status&token=' . $this->session->data['token'] . $url);
		}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'localisation/stock_status&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = HTTPS_SERVER . 'localisation/stock_status/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'localisation/stock_status/delete&token=' . $this->session->data['token'] . $url;	
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'icon' => 'icon-pencil',
                'class' => 'btn-info',
				'href' => HTTPS_SERVER . 'localisation/stock_status/update&token=' . $this->session->data['token'] . '&stock_status_id=' . $result['stock_status_id'] . $url
			);
		...
		$this->data['sort_name'] = HTTPS_SERVER . 'localisation/stock_status&token=' . $this->session->data['token'] . '&sort=name' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'localisation/stock_status&token=' . $this->session->data['token'] . $url . '&page={page}';
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['stock_status_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'localisation/stock_status/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'localisation/stock_status/update&token=' . $this->session->data['token'] . '&stock_status_id=' . $this->request->get['stock_status_id'] . $url;
		}
			
		$this->data['cancel'] = HTTPS_SERVER . 'localisation/stock_status&token=' . $this->session->data['token'] . $url;
		
//		$this->load->model('localisation/language');
		$oLanguages = Make::a('localisation/language')->create();
		$this->data['languages'] = $oLanguages->getLanguages();
...
//		$this->load->model('catalog/product');
		...
		foreach ($this->request->post['selected'] as $stock_status_id) {
			...
			$product_total = $oProd->getTotalProductsByStockStatusId($stock_status_id);
