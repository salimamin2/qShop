...
class ControllerPaymentCheque extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			Make::a('setting/setting')->create()->editSetting('cheque', $this->request->post);				
		...
		$this->data['tab_general'] = $this->language->get('tab_general');
 		...
 		if (isset($this->error['payable'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'payment/cheque&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = HTTPS_SERVER . 'payment/cheque&token=' . $this->session->data['token'];
