...
class ControllerSaleVoucher extends Controller {
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->redirect(makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    protected function getList() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => makeUrl('common/home', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => false
        );

        $this->data['breadcrumbs'][] = array(
            'text'      => $this->language->get('heading_title'),
            'href'      => makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = makeUrl('sale/voucher/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['delete'] = makeUrl('sale/voucher/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//d($results,1);
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'icon' => 'icon-pencil',
                'class' => 'btn-info', 
                'href' => makeUrl('sale/voucher/update', 'token=' . $this->session->data['token'] . '&voucher_id=' . $result['voucher_id'] . $url, 'SSL')
            );
            ...
            $this->data['vouchers'][] = array(
                ...
                'created_on' => date($this->language->get('date_format_short'), strtotime($result['created_on'])),
        ...
        $this->data['column_created_on'] = $this->language->get('column_date_added');
        ...
        $this->data['sort_code'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.code' . $url, 'SSL');
        $this->data['sort_from'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.from_name' . $url, 'SSL');
        $this->data['sort_to'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.to_name' . $url, 'SSL');
        $this->data['sort_theme'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=theme' . $url, 'SSL');
        $this->data['sort_amount'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.amount' . $url, 'SSL');
        $this->data['sort_status'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.created_on' . $url, 'SSL');
        $this->data['sort_created_on'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.created_on' . $url, 'SSL');
...
//d($voucher_total,1);
       /* $pagination = new Pagination();
        ...
        $pagination->url = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
//d($sort,1);
    ...
    protected function getForm() {
        ...
        if (!isset($this->request->get['voucher_id'])) {
            $this->data['action'] = makeUrl('sale/voucher/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = makeUrl('sale/voucher/update', 'token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . $url, 'SSL');
        }

        $this->data['cancel'] = makeUrl('sale/voucher', 'token=' . $this->session->data['token'] . $url, 'SSL');
    ...
    /*    if (($this->request->post['code']) < 3 || ($this->request->post['code']) > 10) {
...
*/
        ...
        if (count($this->request->post['to_name']) < 1 || count($this->request->post['to_name']) > 64) {
            $this->error['to_name'] = $this->language->get('error_to_name');
        }

        if (($this->request->post['to_email']) > 96 || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['to_email'])) {
            $this->error['to_email'] = $this->language->get('error_email');
        }

        if (count($this->request->post['from_name']) < 1 || count($this->request->post['from_name']) > 64) {
            $this->error['from_name'] = $this->language->get('error_from_name');
        }

        if (($this->request->post['from_email']) > 96 || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['from_email'])) {
    ...
    public function send() {
        $this->language->load('sale/voucher');
//d($this->request->get['voucher_id'],1);
        $json = array();
