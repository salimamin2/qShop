...
class ControllerSaleVoucherTheme extends Controller { 
	...
	protected function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => makeUrl('sale/voucher_theme', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);

		$this->data['insert'] = makeUrl('sale/voucher_theme/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = makeUrl('sale/voucher_theme/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'icon' => 'icon-pencil',
                'class' => 'btn-info',
				'href' => makeUrl('sale/voucher_theme/update', 'token=' . $this->session->data['token'] . '&voucher_theme_id=' . $result['voucher_theme_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = makeUrl('sale/voucher_theme', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		...
		$pagination->url = makeUrl('sale/voucher_theme', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_theme_id'])) {
			$this->data['action'] = makeUrl('sale/voucher_theme/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = makeUrl('sale/voucher_theme/update', 'token=' . $this->session->data['token'] . '&voucher_theme_id=' . $this->request->get['voucher_theme_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = makeUrl('sale/voucher_theme', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//d($this->data['voucher_theme_description'],1);
