	...
	private $error = array();
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect(HTTPS_SERVER . 'sale/coupon&token=' . $this->session->data['token'] . $url);
    	}
  	...
  	private function getList() {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'sale/coupon&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = makeUrl('sale/coupon/insert');
		$this->data['delete'] = makeUrl('sale/coupon/delete');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'icon' => 'icon-pencil',
                'class' => 'btn-info',
				'href' => makeUrl('sale/coupon/update',array('coupon_id=' . $result['coupon_id']))
			);
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['coupon_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'sale/coupon/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'sale/coupon/update&token=' . $this->session->data['token'] . '&coupon_id=' . $this->request->get['coupon_id'] . $url;
		}
		
		$this->data['cancel'] = HTTPS_SERVER . 'sale/coupon&token=' . $this->session->data['token'] . $url;
...
//		$this->load->model('localisation/language');
		$oLang = Make::a('localisation/language')->create();
    	$this->data['languages'] = $oLang->getLanguages();
...
//		$this->load->model('catalog/category');
  	...
  	private function validateForm() {
        ...
        if(trim($this->request->post['date_start']) == "") {
            $this->error['date_start'] = $this->language->get('error_date_start');
        }

        if(trim($this->request->post['date_end']) == "" || strtotime($this->request->post['date_end']) < strtotime($this->request->post['date_start'])) {
...
//		$this->load->model('catalog/product');
		...
		$results = $oProd->getProductsByCategoryId($category_id);
...
//		$this->load->model('catalog/product');
		...
		foreach ($products as $product_id) {
			$product_info = $oProd->getProduct($product_id);
