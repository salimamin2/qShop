...
class ModelReportCustomer extends Model {
	public function getOrders($data = array()) { 
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(o.date_added) >= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_start']))) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(o.date_added) <= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_end']))) . "'";
		}
	...
	public function getRewardPoints($data = array()) { 
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(cr.date_added) >= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_start']))) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(cr.date_added) <= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_end']))) . "'";
		}
	...
	public function getTotalRewardPoints() {
		...
		if (!empty($data['filter_date_start'])) {
			$implode[] = "DATE(cr.date_added) >= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_start']))) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = "DATE(cr.date_added) <= '" . $this->db->escape(date('Y-m-d',strtotime($data['filter_date_end']))) . "'";
		}
