...
class ModelSaleVoucher extends Model {
	public function addVoucher($data) {
		/*$this->db->query("INSERT INTO " . DB_PREFIX . "voucher SET code = '" . $this->db->escape($data['code']) . "', from_name = '" . $this->db->escape($data['from_name']) . "', from_email = '" . $this->db->escape($data['from_email']) . "', to_name = '" . $this->db->escape($data['to_name']) . "', to_email = '" . $this->db->escape($data['to_email']) . "', voucher_theme_id = '" . (int)$data['voucher_theme_id'] . "', message = '" . $this->db->escape($data['message']) . "', amount = '" . (float)$data['amount'] . "', status = '" . (int)$data['status'] . "', created_on = NOW()");*/

        $oModel = ORM::for_table('voucher')->create();
        $oModel->code=$this->db->escape($data['code']);
        $oModel->from_name=$this->db->escape($data['from_name']);
        $oModel->from_email=$this->db->escape($data['from_email']);
        $oModel->to_name=$this->db->escape($data['to_name']);
        $oModel->to_email=$this->db->escape($data['to_email']);
        $oModel->voucher_theme_id=$this->db->escape($data['voucher_theme_id']);
        $oModel->message=$this->db->escape($data['message']);
        $oModel->amount=$this->db->escape($data['amount']);
        $oModel->status=$this->db->escape($data['status']);
        $oModel->created_on=date("Y-m-d");
    ...
    public function getVouchers($data = array()) {
        ...
        if (isset($data['order']) && ($data['order'] == 'DESC')) {
            ...
            $oVouchers =Make::a('voucher')
            ->table_alias('v')
            ->select_expr('v.*')
            ->raw_query('select v.*,(SELECT vtd.name FROM voucher_theme_description vtd WHERE vtd.voucher_theme_id = v.voucher_theme_id) AS theme from voucher v ORDER BY v.created_on'.$sql)
...
//d($voucher_info,1);
	...
	public function getVoucherHistories($voucher_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT vh.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, vh.amount, vh.created_on FROM " . DB_PREFIX . "voucher_history vh LEFT JOIN `" . DB_PREFIX . "order` o ON (vh.order_id = o.order_id) WHERE vh.voucher_id = '" . (int)$voucher_id . "' ORDER BY vh.created_on ASC LIMIT " . (int)$start . "," . (int)$limit);
